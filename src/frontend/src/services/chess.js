...
export const chessService = {
    ...
    async loadGame(gameId) {
        try {
            const response = await api.get(`/api/chess/games/${gameId}`)
    ...
    async getGameMoves(gameId) {
        try {
            const response = await api.get(`/api/chess/games/${gameId}/moves`)
            return response.data
        } catch (error) {
    ...
    async analyzePosition(fen, depth = 10) {
        try {
            const response = await api.post('/api/chess/analyze', { fen, depth })
            return response.data
        } catch (error) {
    ...
    async validateMove(gameId, move) {
        try {
            const response = await api.post(`/api/chess/games/${gameId}/validate-move`, { move })
            return response.data
        } catch (error) {
