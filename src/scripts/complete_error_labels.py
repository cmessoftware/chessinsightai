...
Usage:
    python complete_error_labels.py --limit 50
    python complete_error_labels.py --limit 500 --workers 2
"""
    ...
    try:
        ...
        if args.workers == 1:
            ...
            for i, game_data in enumerate(games_data, 1):
                ...
                logger.info(f"✅ Completed game {result['game_id']}: {updated_count} features updated")
        
        else:
            # Parallel processing
            logger.info(f"[PARALLEL] Processing with {args.workers} workers...")
            
            with ProcessPoolExecutor(max_workers=args.workers) as executor:
                # Submit all games
                future_to_game = {
                    executor.submit(process_game_for_error_labels, game_data): game_data[0]
                ...
                for future in as_completed(future_to_game):
                    ...
                    try:
                        ...
                        logger.error(f"[ERROR] Error processing future for game {game_id}: {e}")
                        total_errors += 1
