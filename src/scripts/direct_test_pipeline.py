    ...
    try:
        ...
        for parquet_path, source, limit in parquet_files:
            if os.path.exists(parquet_path):
                logger.info(
                    f"📖 Loading {source} games from {parquet_path} (limit: {limit})"
                )
                ...
                try:
                    df.to_sql(
                        "games", engine, if_exists="append", index=False, method="multi"
                    )
...
def generate_features_sample():
    ...
    try:
        ...
        from scripts.generate_features_with_tactics import (
            main as generate_features_main,
        )
