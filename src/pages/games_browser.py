...
def display_games_table(df: pd.DataFrame, total_count: int, page: int, per_page: int):
    ...
    if selected_indices:
        ...
        with col1:
            st.info(f"✅ {len(selected_game_ids)} partidas seleccionadas")

            if st.button("📋 Ver detalles", disabled=len(selected_game_ids) != 1):
...
def display_game_details(game_id: str):
    ...
    with info_col1:
        st.write(f"**🎯 Apertura:** {game_data['opening_name'] or 'N/A'}")
        st.write(f"**⏱️ Control de tiempo:** {game_data['time_control'] or 'N/A'}")
        st.write(f"**📁 Fuente:** {game_data['source'] or 'N/A'}")

    with info_col2:
        st.write(f"**🔚 Terminación:** {game_data['termination'] or 'N/A'}")
        st.write(f"**📝 Jugadas de apertura:** {game_data['opening_ply'] or 'N/A'}")

    # Game moves
    if game_data.get("moves_text"):
        st.markdown("### ♟️ Movimientos")
        st.code(game_data["moves_text"], language="text")
    else:
        st.warning("⚠️ No hay movimientos disponibles para esta partida")

    # Final position
    if game_data.get("final_position"):
        st.markdown("### 🏁 Posición Final")
        st.code(game_data["final_position"], language="text")
