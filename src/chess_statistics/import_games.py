...
from chess_statistics.db import InsertResult, StatisticsRepository
from chess_statistics.evals import StockfishAnalysisService, persist_evals
from chess_statistics.filters import filter_import_game, total_move_count
from chess_statistics.game_id import identity_from_ndjson
...
def game_row_from_ndjson(game: dict[str, Any], username: str) -> GameRow:
    ...
    platform = str(game.get("source_platform") or SOURCE_LICHESS)
    lichess_id = identity.lichess_id if platform == SOURCE_LICHESS else None
    external = str(game.get("external_game_id") or lichess_id or "").strip() or None
    source_url = str(game.get("source_url") or "").strip() or (
        f"https://lichess.org/{lichess_id}" if lichess_id else None
    )
