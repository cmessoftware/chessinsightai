...
from chess_statistics.filters import (
...
from chess_statistics.sources import SOURCE_CHESSCOM, SOURCE_LICHESS, SOURCE_PGN

_LICHESS_GAME = re.compile(
    r"lichess\.org/(?:embed/)?([A-Za-z0-9]{8,12})",
    re.I,
)
_CHESSCOM_GAME = re.compile(
    r"chess\.com/(?:game/live|live/game|game)/([0-9]+)",
    re.I,
)
...
def detect_source_platform(game: chess.pgn.Game, pgn_text: str) -> str:
    blob = _combined_text(game, pgn_text).lower()
    if "chess.com" in blob:
        return SOURCE_CHESSCOM
    if is_lichess_export(game, pgn_text):
        return SOURCE_LICHESS
    return SOURCE_PGN


def source_url_for(game: chess.pgn.Game, pgn_text: str, platform: str, external_id: str | None) -> str | None:
    for key in ("Link", "Site"):
        raw = _header(game, key)
        if raw.startswith("http"):
            return raw
    if platform == SOURCE_LICHESS and external_id:
        return f"https://lichess.org/{external_id}"
    if platform == SOURCE_CHESSCOM and external_id:
        return f"https://www.chess.com/game/live/{external_id}"
    match = _CHESSCOM_GAME.search(_combined_text(game, pgn_text))
    if match:
        return f"https://www.chess.com/game/live/{match.group(1)}"
    return None


def chesscom_id_from_pgn(game: chess.pgn.Game, pgn_text: str) -> str | None:
    match = _CHESSCOM_GAME.search(_combined_text(game, pgn_text))
...
def ndjson_from_pgn_game(
    game: chess.pgn.Game,
    pgn_text: str | None = None,
    *,
    require_lichess: bool = True,
) -> dict[str, Any]:
    ...
    if getattr(game, "errors", None):
        return _skip_payload(REASON_PGN_ERRORS, game, pgn_text)
    platform = detect_source_platform(game, pgn_text)
    if require_lichess and platform != SOURCE_LICHESS:
    ...
    lichess_id = lichess_id_from_pgn(game, pgn_text) if platform == SOURCE_LICHESS else None
    chesscom_id = chesscom_id_from_pgn(game, pgn_text) if platform == SOURCE_CHESSCOM else None
    external = lichess_id or chesscom_id
    payload: dict[str, Any] = {
        ...
        "source_url": source_url_for(game, pgn_text, platform, external),
...
def iter_pgn_file(path: str | Path, *, require_lichess: bool = True) -> Iterator[dict[str, Any]]:
    ...
    while True:
        ...
        yield ndjson_from_pgn_game(parsed, pgn_text=chunk, require_lichess=require_lichess)
