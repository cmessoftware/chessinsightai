#!/usr/bin/env python3
"""
Validation script for the ML prep        try:
            preprocessor = ChessMLPreprocessor()
            processed_df = preprocessor.fit_transform(df.copy(), source=source)essing pipeline with tactical features.
...
def main():
    ...
    for source in sources:
        ...
        try:
            preprocessor = ChessMLPreprocessor()
            processed_df = preprocessor.fit_transform(df.copy(), source=source)
