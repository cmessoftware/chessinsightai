        ...
        if os.path.exists(unified_file):
                ...
                for source, target in TARGET_SAMPLES.items():
                    ...
                    if len(source_df) > 0:
                        ...
                        for _, row in sampled.iterrows():
                            game_id = row.get('game_id', f"{source}_{len(game_records)}")
            ...
            for parquet_path, source in individual_sources:
                if os.path.exists(parquet_path) and source in TARGET_SAMPLES:
                    ...
                    logger.info(f"📖 Loading {source} from {parquet_path} (target: {target:,})")
    ...
    try:
        ...
        for source in TARGET_SAMPLES.keys():
            ...
            cmd = [
                ...
                '--source', source,
                '--max-games', '999999',  # Process all imported games
                '--workers', '6',
                '--chunk-size', '1000'
            ]
