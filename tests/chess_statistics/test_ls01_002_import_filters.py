...
def test_nine_full_moves_seventeen_plies_is_skipped():
    """DGzeuss-style: FEN fullmove 9 / 17 plies is under 10 jugadas."""
    game = _human(plies=17)
    result = filter_import_game(game)
    assert result.keep is False
    assert result.reason == REASON_TOO_SHORT
    assert result.move_count == 17


def test_ten_full_moves_are_kept():
    game = _human(plies=20)
    result = filter_import_game(game)
