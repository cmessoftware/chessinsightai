...
from chess_statistics.db import (  # noqa: E402
    DEFAULT_DB_PATH,
...
def _repo(tmp_path: Path) -> tuple[sqlite3.Connection, StatisticsRepository]:
    conn = connect(tmp_path / "chess_statistics.sqlite")
    init_schema(conn)
...
def test_sqlite_url_and_default_path():
    assert sqlite_path_from_url("sqlite:///data/chess_statistics.sqlite") == Path(
        "data/chess_statistics.sqlite"
    )
    assert sqlite_path_from_url("sqlite:///C:/tmp/chess_statistics.sqlite") == Path(
        "C:/tmp/chess_statistics.sqlite"
    )
    assert DEFAULT_DB_PATH == Path("data") / "chess_statistics.sqlite"
